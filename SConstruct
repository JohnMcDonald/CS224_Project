...
#dist = env.Command('dist', [], 'tar -cvzf scons.tar ma.c cmaTest.c cma.c cma.h SConstruct')
...
if 'removeTar' in COMMAND_LINE_TARGETS:
	env.Command('removeTar', ['scons.tar'], 'rm scons.tar')

if 'removeDir' in COMMAND_LINE_TARGETS:
	env.Command('removeDir', ['./LIBCMA'], 'rm -r ./LIBCMA')
